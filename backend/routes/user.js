			...
			nexmo.verify.request({number: phoneNumber, brand:"EasyVote", code_length:4},(err,result)=>{ //sets pin expiry to 240 . User has 2 tries before pin is changed
				...
				if(err){
					...
					if(result.status == '0'){
						...
						user.meta.tokenPasscode = result.request_id;
						user.save();//updates token
...
router.post('/verify2', async (req,res)=>{
	...
	let userID = ObjectID("5bff32f01b25460a89800c8b");
	let verificationMethod = req.body.verificationMethod;  //0 for Phone and 1 for Email

	let user = await User.findById(userID).exec();
	console.log(userID, pin, verificationMethod);
	console.log(user)
	if(!user) {
		return res.send({error: "User account does not exist"})
	}
